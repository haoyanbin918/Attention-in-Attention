...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, n_segments, num_classes=1000, fold_div=8, place='blockres'):
        self.inplanes = 64
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, n_seg=n_segments[1], fold_d=fold_div, pla=place)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, n_seg=n_segments[2], fold_d=fold_div, pla=place)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, n_seg=n_segments[3], fold_d=fold_div, pla=place)
        #
    ...
    def _make_layer(self, block, planes, blocks, stride=1, n_seg=8, fold_d=8, pla=None):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, n_seg, fold_d, pla))
        ...
        for i in range(1, blocks):
            if i % n_round == 0:
                layers.append(block(self.inplanes, planes, n_segment=n_seg, fold_div=fold_d, place=pla))
            else:
...
if __name__ == "__main__":
    ...
    net = resnet50(False, n_segment=8, fold_div=8).cuda()
    print(net)
