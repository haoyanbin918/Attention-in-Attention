...
class VideoNet(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                ...
                fc_lr5=False, non_local=False, element_filter=False, cdiv=2, target_transforms=None):
        ...
        if 'resnet' in backbone:
            ...
            if self.net == 'C2D':
                ...
                self.base_model = getattr(TSM, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                    n_segment=self.num_segments, fold_div=8, place='blockres')

            elif 'C2D' in self.net:
                print('=> base model: {}, with backbone: {}'.format(self.net, backbone))
                C2D_V = import_module('nets.'+self.net)
                self.base_model = getattr(C2D_V, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                    num_segments=self.num_segments)

            elif 'TSM' in self.net:
                print('=> base model: {}, with backbone: {}'.format(self.net, backbone))
                TSM_V = import_module('nets.'+self.net)
                self.base_model = getattr(TSM_V, backbone)(pretrained=True if self.pretrain == 'imagenet' else False,
                    num_segments=self.num_segments, fold_div=8, place='blockres')
            else:
                raise ValueError('model name is not right %s'%self.net)
                # assert self.net in ['E33D', 'E31D', 'T33D', 'T31D', 'S33D', 'S31D', 'CLLD']
                # base_model_name = 'nets.STC_' + self.net
                # print('=> base model: {}, with backbone: {}'.format(base_model_name, backbone))
                # STC = import_module(base_model_name)
                # self.base_model = getattr(STC, backbone)(alpha=self.alpha, beta=self.beta, use_ef=self.element_filter)
            #
